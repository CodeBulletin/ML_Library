...
class NeuralNetwork(optimizers.optimizers):
    def __init__(self, input_neurons=0, network_shape=None, learning_rate=0.1, beta=0.9, gamma=0.999, epsilon=1e-8,
        ...
        for i in range(len(self.shape) - 1):
            ...
            self.weights.append(
                np.random.standard_normal(self.weights_shape[i]) / (self.weights_shape[i][1] ** 0.5))
            self.biases.append(np.random.standard_normal([self.shape[i + 1], 1]))
    ...
    def mutate(self, mutation_rate, mutation_ratio):
        def internal(x, mr, mro, element):
            ...
            if temp1 < mr:
                ...
                if temp2 < mro:
                    return np.random.standard_normal()/(self.weights_shape[element][1]**0.5)
                else:
                    return x+np.random.uniform(-x/100, x/100)
            else:
                return x
        randomize = np.vectorize(internal)
    ...
    def crossover(self, parent2):
        ...
        for i in range(len(self.weights)):
            ...
            if r < 0.5:
                ...
                child.weights[i] = np.copy(parent2.weights[i])
                child.biases[i] = np.copy(parent2.biases[i])
        return child
    ...
    def loss(self, x, y):
        if self.do_regularization:
            return (1 / len(x[0])) * (np.sum(self.cost(x, y))
    ...
    def print_accuracy(self, x, y, do_return=False, do_print=True, find_loss=False, in_percent=False):
        ...
        output = np.argmax(y_hat, axis=0)
        target = np.argmax(y, axis=0)
        correct = np.equal(output, target)
    ...
    def predict(self, x):
        if self.do_norm:
            ...
            output = x.copy()
        for i in range(len(self.weights)):
            output = self.activation[i](np.matmul(self.weights[i], output) + self.biases[i])
        return output
    ...
    def train(self, x_train, y_train, x_val=None, y_val=None, epochs=1,
        ...
        for _ in range(epochs):
            ...
            if shuffle_data:
                shuffle = np.random.permutation(x_t.shape[1])
                x_t = x_t[:, shuffle]
                y_t = y_t[:, shuffle]
    ...
    def BatchGrad(self, x, y):
        ...
        if self.optimizer_key == "Momentum" or self.optimizer_key == "RMSprop":
            Vdw = [np.zeros(i.shape) for i in self.weights]
            Vdb = [np.zeros(i.shape) for i in self.biases]
        elif self.optimizer_key == "Adam":
            Vdw = [np.zeros(i.shape) for i in self.weights]
            Vdb = [np.zeros(i.shape) for i in self.biases]
            Sdw = [np.zeros(i.shape) for i in self.weights]
            Sdb = [np.zeros(i.shape) for i in self.biases]
        for i in range(len(self.weights)):
        ...
        for i in reversed(range(len(self.weights))):
            ...
            if not self.do_regularization:
                dW = (1/len(x[0]))*np.matmul(dZ, a[i].T)
            ...
            error = np.matmul(self.weights[i].T, dZ)
    ...
    def MiniBatchGrad(self, x_t, y_t, batch_size, on_batch_end, on_batch_start):
        ...
        if self.optimizer_key == "Momentum" or self.optimizer_key == "RMSprop":
            ...
            Sdw = [np.zeros(i.shape) for i in self.weights]
            Sdb = [np.zeros(i.shape) for i in self.biases]
        batches = int(np.ceil(len(x_t[0])/batch_size))
        for batch in range(batches):
            ...
            for i in reversed(range(len(self.weights))):
                ...
                if not self.do_regularization:
                    dW = (1/batch_size)*np.matmul(dZ, a[i].T)
    ...
    def StochasticGrad(self, x_t, y_t):
        ...
        for j in range(len(x_t[0])):
            ...
            for i in range(len(self.weights)):
                z.append(np.matmul(self.weights[i], a[i]) + self.biases[i])
                a.append(self.activation[i](z[i]))
            error = self.d_cost(a[len(a)-1], np.array([y[0:][j]]).T)
            for i in reversed(range(len(self.weights))):
                if not self.Activations[i] == SoftMax:
                    dz = error * self.derivative[i](z[i])
                else:
                ...
                if not self.do_regularization:
                    dw = np.matmul(dz, a[i].T)
                    db = dz
                else:
                    dw = np.matmul(dz, a[i].T) + self.regularization.fn2(self.weights[i], self.lam, len(x_t[0]))
...
def load(file_name):
    with open(file_name, "r") as file:
        ...
        weights = [np.array(i) for i in data["weights"]]
        model.weights = weights
        biases = [np.array(i) for i in data["biases"]]
        model.biases = biases
        model.activation = [model.ActivationList[i].fn for i in model.Activations]
        model.derivative = [model.ActivationList[i].d for i in model.Activations]
        model.optimizer = model.OptimizerList[model.optimizer_key]
